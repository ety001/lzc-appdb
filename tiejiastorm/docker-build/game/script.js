...
class MyClass {
    ...
    async LoadEmulator(byteArray){
        ...
        // === CD preload from server (injected: supports .iso directly, or .cue + referenced files) ===
        if (this.rivetsData.settings.DEFAULTCD && this.img_loaded)
        {
            try {
                let cdBase = this.rivetsData.settings.DEFAULTCD;
                let cdPath = cdBase.substring(0, cdBase.lastIndexOf('/') + 1) || '';
                let cdFile = cdBase.split('/').pop();
                let isCue = cdFile.toLowerCase().endsWith('.cue');
                if (isCue) {
                    let cdResp = await fetch(cdBase);
                    if (cdResp.ok) {
                        let cdText = await cdResp.text();
                        let fileRe = /FILE\s+"([^"]+)"/gi;
                        let m, refs = [];
                        while ((m = fileRe.exec(cdText)) !== null) refs.push(m[1]);
                        for (let rf of refs) {
                            let bn = rf.split('/').pop();
                            let url = cdPath + rf;
                            console.log('CD preload', url);
                            try {
                                let r2 = await fetch(url);
                                if (r2.ok) {
                                    let buf = await r2.arrayBuffer();
                                    let arr = new Uint8Array(buf);
                                    try { Module.FS.writeFile('/' + bn, arr); } catch(e) { console.log('write fail', bn); }
                                }
                            } catch(e) { console.log('fetch fail', url); }
                        }
                        // rewrite cue: filenames -> basenames
                        let flatCue = cdText.replace(/FILE\s+"([^"]+)"/gi, function(mm, fn) {
                            return 'FILE "' + fn.split('/').pop() + '"';
                        });
                        let cueArr = new Uint8Array(flatCue.length);
                        for (let i = 0; i < flatCue.length; i++) cueArr[i] = flatCue.charCodeAt(i) & 0xff;
                        try { Module.FS.writeFile('/' + cdFile, cueArr); } catch(e) { console.log('cue write fail'); }
                    }
                } else {
                    // .iso direct: just write the single file to root
                    let url = cdBase;
                    console.log('CD preload (iso)', url);
                    let r2 = await fetch(url);
                    if (r2.ok) {
                        let buf = await r2.arrayBuffer();
                        let arr = new Uint8Array(buf);
                        try { Module.FS.writeFile('/' + cdFile, arr); } catch(e) { console.log('iso write fail', cdFile); }
                    }
                }
            } catch(e) { console.log('CD preload error', e); }
        }
        ...
        // CD auto-load: mount if DEFAULTCD set (flat cue at root)
        if (this.rivetsData.settings.DEFAULTCD && !this.hasBinCue)
        {
            let cueBase = this.rivetsData.settings.DEFAULTCD.split('/').pop();
            multiFileScript += 'imgmount d "' + cueBase + '" -t cdrom -ide 2m\r\n';
        }
...
// === 浏览器热键处理 v2 ===
// 1) preventDefault 挡掉浏览器默认行为(切标签等)，但放行事件给模拟器(编队键可用)
// 2) 全屏时 Keyboard Lock 拦截浏览器保留键(Ctrl+W 等)
// 3) ` 或 Ctrl+Q 主动退出鼠标指针锁(替代 Esc，游戏内 Esc 不受影响)
(function(){
    ...
    window.addEventListener('keydown', function(e){
        ...
        // 退出鼠标捕获：反引号 或 Ctrl+Q
        if (e.key === '`' || (e.ctrlKey && e.code === 'KeyQ')) {
        ...
        if (e.ctrlKey || e.metaKey || e.altKey) {
            e.preventDefault(); // 挡浏览器默认动作，事件继续传给模拟器
        }
    }, true);
    ...
    if (navigator.keyboard && navigator.keyboard.lock) {
        document.addEventListener('fullscreenchange', function(){
            if (document.fullscreenElement) {
                ...
                try { navigator.keyboard.unlock(); } catch(err) {}
            }
        });
    }
})();
